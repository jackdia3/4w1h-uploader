...
def get_and_save_images(soup, code, images_dir_webp):
    images_info = {}
    MAIN_KEYS = ["main", "mainimg", "_hotsand_mainimg", "_flyer_mainimg"]
    for key, selector in IMAGE_SELECTORS.items():
        if key == "backgrounds":
            continue
        img_list = get_images_with_alt(soup, selector, key)
        normed_list = []
        for img in img_list:
            # 用原始檔名產生 webp 檔名
            orig_name, _ = os.path.splitext(os.path.basename(img['url']))
            usage = "main" if key in MAIN_KEYS else key
            webp_filename = f"uncle-benny-{code}_{orig_name}.webp"
            webp_path = images_dir_webp / webp_filename
            ok = download_and_save_webp(img['url'], webp_path)
            img['filename'] = webp_filename
            img['url'] = None  # 產生 HTML 時自動用 CDN + filename
            img['status'] = 'ok' if ok else 'error'
            normed_list.append(img)
        info_key = "main" if key in MAIN_KEYS else key
        if len(normed_list) == 1:
            images_info[info_key] = normed_list[0]
        elif normed_list:
            images_info[info_key] = normed_list
    return images_info
